...
- **The Gemini setup help names the `.env` this launch actually reads** (`@mulmoclaude/core`, #3231, closes #2626) — the instructions said to put `GEMINI_API_KEY` in a `.env` "in the directory you launch MulmoClaude from", which is only true from a terminal. Started from the icon there is no launch directory — macOS starts apps in `/` — so the app starts from home and reads `~/.env`, and a shell `export` never reaches it because the launcher takes PATH from the login shell and nothing else. The help now splits the two routes, points at Settings → Gemini for the exact path this launch reads, and notes that restarting from the icon means quitting the server first.
...
Ships `@mulmoclaude/accounting-plugin@4.0.0`, `@mulmoclaude/chart-plugin@4.0.0`, `@mulmoclaude/collection-plugin@5.0.0`, `@mulmoclaude/common@1.3.0`, `@mulmoclaude/core@5.0.1`, `@mulmoclaude/form-plugin@2.0.0`, `@mulmoclaude/google-plugin@4.0.0`, `@mulmoclaude/html-plugin@5.0.0`, `@mulmoclaude/markdown-plugin@5.0.0`, `@mulmoclaude/markdown-utils@3.0.0`, `@mulmoclaude/mulmoscript-plugin@5.0.0`, `@mulmoclaude/shapescript-plugin@7.0.0`, `@mulmoclaude/spotify-plugin@2.0.1`, `@mulmoclaude/x-plugin@1.0.4`.
